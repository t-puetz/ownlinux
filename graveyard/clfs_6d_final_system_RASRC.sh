...
#libelf 32-bit
mkdir libelf && tar xf elfutils-*.tar.* -C libelf --strip-components 1
cd libelf
...
make -C libelf install PREFIX=/usr LIBDIR=/usr/lib
...
make -C libelf install PREFIX=/usr LIBDIR=/usr/lib64
...
python3 setup.py install --verbose --prefix=/usr --install-lib=/usr/lib64/python3.7/site-packages --optimize=1
...
#Kmod 32-bit
mkdir kmod && tar xf kmod-25.tar.* -C kmod --strip-components 1
cd kmod
...
CC="gcc ${BUILD64}" ./configure --prefix=/usr --sysconfdir=/etc \
    ...
    --disable-keymap --disable-gudev --disable-static --disable-gtk-doc-html \
    --with-firmware-path=/lib/firmware --enable-libkmod --enable-manpages \
    --config-cache
...
LD_LIBRARY_PATH=/tools/lib65 udevadm hwdb --update
