...
#LINUX KERNEL
mkdir linux && tar xf linux-4.15*.tar.* -C linux --strip-components 1
cd linux

rm -rf /lib/modules/4.15.*
rm -rf /lib/modules/*4.15.*
rm -rf /boot/efi/System.map-4.15.*
rm -rf /boot/efi/vmlinuz-4.15.*
rm -rf /lib/firmware

make mrproper
cp ${CLFSSOURCES}/kernel415.conf ${CLFSSOURCES}/linux/.config
...
cp -v arch/x86_64/boot/bzImage /boot/efi/vmlinuz-4.15.7
cp -v System.map /boot/efi/System.map-4.15.7
cp -v .config /boot/efi/config-4.15.7

cd ${CLFSSOURCES}

#Copy source folder to /lib/modules
mv ${CLFSSOURCES}/linux /lib/modules/CLFS-4.15.7-headers

#Properly link the new kernel source folder path to subdirectories
# build/ and source/
unlink /lib/modules/4.15.7-CLFS-SYSVINIT-SVN-x86_64/build
unlink /lib/modules/4.15.7-CLFS-SYSVINIT-SVN-x86_64/source
ln -sfv /lib/modules/CLFS-4.15.7-headers /lib/modules/4.15.7-CLFS-SYSVINIT-SVN-x86_64/build
ln -sfv /lib/modules/CLFS-4.15.7-headers /lib/modules/4.15.7-CLFS-SYSVINIT-SVN-x86_64/source
...
echo "title   Cross Linux from Scratch (4.15.7)" >> clfs-uefi.conf
echo "linux   /vmlinuz-4.15.7" >> clfs-uefi.conf
...
if [[ $CPUVENDOR == "AuthenticAMD" ]]; then
  ...
  if [[ $CPUFAMILY == "23" ]]; then # This is the Ryzen Gen 1 Family
  ...
  wget https://salsa.debian.org/hmh/amd64-microcode/blob/master/microcode_amd_fam17h.bin
